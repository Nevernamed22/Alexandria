...
public static class TexturePacker
{
	...
	private static Rect[] PackTextures(Texture2D texture, Texture2D[] sprites, int padding, int width, int height, int maxSize, bool forceSquare)
	{
		...
		for (int i = 0; i < sprites.Length; i++)
		{
			...
			var packingMethodConfig = TexturePacker.dfTexturePackingMethod.RectBestAreaFit;//(dfTexturePacker.dfTexturePackingMethod)EditorPrefs.GetInt("DaikonForge.AtlasPackingMethod", (int)dfTexturePacker.dfTexturePackingMethod.RectBestAreaFit);
			Rect rect = bp.Insert(
				spriteWidth,
				spriteHeight,
				packingMethodConfig
		...
		var extrudeEdges = false;//EditorPrefs.GetBool("DaikonForge.AtlasExtrudeSprites", false) && padding > 0;
		for (int i = 0; i < sprites.Length; i++)
		{
